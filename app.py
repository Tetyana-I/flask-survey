...
def start_survey():
    ...
    # current_survey=surveys[f"{request.form.get('choice')}"] #current survey object
...
def handling_start():
    """ Set a current session to an empty list and redirect to the start of the survey """
    session['responses'] = []
    # answers = session.get('responses')
    # if (not answers) or (len(answers) >= session['num_of_questions']):
    #     session['responses'] = []
    #     question = 0
    # else:
